...
// 由python.cfg 文件生成
#include <paramDynamicSet/paramDynamicSetPYConfig.h>
// 服务相关头文件
...
public:
    callbackClass(){
        ...
        // 服务客户端初始化
        intClient = handler.serviceClient<paramDynamicSet::service_int>("int_service_channel");
        floatClient = handler.serviceClient<paramDynamicSet::service_float>("float_service_channel");
    ...
    void callback(paramDynamicSet::paramDynamicSetPYConfig &config, uint32_t level){
    // ROS_INFO("%d, %f, %s, %s, %d",
    // config.int_param,
    // config.double_param,
...
private:
    ...
    paramDynamicSet::paramDynamicSetPYConfig last;
    // 服务客户端声明
    ...
    // 动态参数调节服务端实例
    dynamic_reconfigure::Server<paramDynamicSet::paramDynamicSetPYConfig> server;
    dynamic_reconfigure::Server<paramDynamicSet::paramDynamicSetPYConfig>::CallbackType func;
    paramDynamicSet::service_int intRqst;
    paramDynamicSet::service_bool boolRqst;
    paramDynamicSet::service_float floatRqst;
    paramDynamicSet::service_string stringRqst;
    paramDynamicSet::service_enum enumRqst;
};
