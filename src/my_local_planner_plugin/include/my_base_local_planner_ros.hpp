...
namespace base_local_planner{
    class myBaseLocalPlannerROS : public nav_core::BaseLocalPlanner {
         ...
         * @return my_local_planner_kernel* 
         */
        my_local_planner_kernel* getPlanner() const;
        ...
        my_local_planner_kernel* tc_; ///< @brief kernel
...
namespace base_local_planner{
    ...
    bool myBaseLocalPlannerROS::checkTrajectory(double vx_samp, double vy_samp, double vtheta_samp, bool update_map){
        ...
        if(costmap_ros_->getRobotPose(global_pose)){
            ...
            return tc_->checkTrajectory(global_pose.pose, base_odom.twist.twist, vx_samp, vy_samp, vtheta_samp);
    ...
    double myBaseLocalPlannerROS::scoreTrajectory(double vx_samp, double vy_samp, double vtheta_samp, bool update_map){
        ...
        if(costmap_ros_->getRobotPose(global_pose)){
            ...
            return tc_->scoreTrajectory(global_pose.pose, base_odom.twist.twist, vx_samp, vy_samp, vtheta_samp);
    ...
    my_local_planner_kernel* myBaseLocalPlannerROS::getPlanner() const {
        return tc_;
    }
    bool myBaseLocalPlannerROS::rotateToGoal(const geometry_msgs::PoseStamped& global_pose, const geometry_msgs::PoseStamped& robot_vel, double goal_th, geometry_msgs::Twist& cmd_vel){
        ...
        bool valid_cmd = tc_->checkTrajectory(global_pose.pose, robot_vel.pose, 0.0, 0.0, v_theta_samp);
        if(valid_cmd){
    ...
    bool myBaseLocalPlannerROS::stopWithAccLimits(const geometry_msgs::PoseStamped& global_pose, const geometry_msgs::PoseStamped& robot_vel, geometry_msgs::Twist& cmd_vel){
        ...
        bool valid_cmd = tc_->checkTrajectory(global_pose.pose, robot_vel.pose, vx, vy, vth);
        if(valid_cmd){
...
namespace base_local_planner{
    ...
    void myBaseLocalPlannerROS::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros){
        ...
        tc_ = new my_local_planner_kernel(*world_model_, *costmap_, footprint_spec_,
        ...
        map_viz_.initialize(name, global_frame_, boost::bind(&my_local_planner_kernel::getCellCosts, tc_, _1, _2, _3, _4, _5, _6));
        initialized_ = true;
